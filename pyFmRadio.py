...
class FMDemod(threading.Thread):
  def run(self):
    ...
    audioFiltStateLplusR = np.zeros((audioDec,int(np.size(audioFilt)/audioDec-1)),dtype=np.complex64);
    audioFiltStateLminusR = np.zeros((audioDec,int(np.size(audioFilt)/audioDec-1)),dtype=np.complex64);
    ...
    rbdsFiltState = np.zeros((rbdsDec,int(np.size(rbdsFilt)/rbdsDec-1)),dtype=np.complex64);
